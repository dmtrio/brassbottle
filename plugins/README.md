...
| `install: \|` | Bash run at **image build** (full network). Required iff a local `command:` entry exists whose `command` is not a **base tool** — a binary the image already provides, listed in `BASE_IMAGE_BINS` (`src/manifest.py`) and installed by the `Dockerfile`. Today that is `mcp-remote` only. Checked per **server**: a plugin running `mcp-remote` for one server and its own binary for another still needs `install:` for the second. Base tools installed above the bake loop in the Dockerfile — `yq`, `mcp-remote`, `herdr` — are available to an `install:` block without declaring anything. Interpreters (`bash`, `python3`) are deliberately **not** base tools — an entry means "exec'ing this *is* the whole server", which is false of a wrapper whose real payload sits in `args`. |
