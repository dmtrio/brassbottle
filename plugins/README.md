...
| [`opencode`](opencode/) | env-only (no server) | — | [README](opencode/README.md) |
...
| [`herdr-auto-title`](herdr-auto-title/) | herdr plugin (baked, linked at build) | — | [README](herdr-auto-title/README.md) |
...
| `install: \|` | Bash run at **image build** (full network). Required iff a local `command:` entry exists whose `command` is not a **base tool** — a binary the image already provides, listed in `BASE_IMAGE_BINS` (`src/manifest.py`) and installed by the `Dockerfile`. Today that is `mcp-remote` only. Checked per **server**: a plugin running `mcp-remote` for one server and its own binary for another still needs `install:` for the second. Separately, binaries the Dockerfile installs above the bake loop — `yq`, `mcp-remote`, `herdr` — are on PATH *at build time* for an `install:` block to call; that is not the same as being a base tool for a `command:` entry (that list is still `mcp-remote` only). Interpreters (`bash`, `python3`) are deliberately **not** base tools — an entry means "exec'ing this *is* the whole server", which is false of a wrapper whose real payload sits in `args`. |
