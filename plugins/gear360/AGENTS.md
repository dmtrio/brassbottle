...
`/opt/plugins/gear360/LOG.md` (repo: `plugins/gear360/LOG.md`) has the full
change history — what broke, what was patched upstream, and why. Read it before
concluding that something here is arbitrary, or before "fixing" a workaround.

The one rule from that history worth repeating here, because it changes what
you do rather than just explaining the past: **a zero exit code from the
stitcher is not evidence of success.** `cv::VideoWriter` silently discarded
every frame for a one-pixel size mismatch and still returned 0, leaving a
structurally valid but empty file. Confirm the output has a video stream.
