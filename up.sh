...
# (below) appends it to every agent's global rules file, so all harnesses read
# it through the same channel. The old /workspace/CLAUDE.md copy is removed so
# Claude does not load the contract twice (it was generated by this script).
