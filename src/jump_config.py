...
# Offset of the jump's static address inside the bridge subnet. .1 is the
# docker-assigned gateway, so .2 is the first free host address. Static so the
# tunnel's target survives a recreate (a DHCP bridge IP does not).
JUMP_HOST_OFFSET = 2
...
def resolve_jump_ip(env: dict[str, str] | None = None) -> str:
    ...
    if override:
        ...
        if addr not in subnet:
            raise JumpConfigError(
                f"{ENV_JUMP_IP} {addr} is outside {ENV_SUBNET} {subnet}"
            )
        ...
        return str(addr)
    candidate = subnet.network_address + JUMP_HOST_OFFSET
    if candidate not in subnet:
        raise JumpConfigError(f"{ENV_SUBNET} {subnet} is too small for a jump address")
...
services:
  {SERVICE_NAME}:
    ...
    environment:
      SSH_AUTHORIZED_KEY: "{authorized_key.strip()}"
