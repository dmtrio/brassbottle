...
# A directory mount, rather than a single file mount: registry refreshes use
# os.replace() for atomicity, and a bind-mounted file would retain its old
# inode inside the running jump container.
REGISTRY_DIRNAME = "registry"
REGISTRY_FILENAME = "bottles"
REGISTRY_MOUNT = "/etc/djinn-jump/registry"
...
def paths(base_path: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "registry_dir": jump_root / REGISTRY_DIRNAME,
        "registry_file": jump_root / REGISTRY_DIRNAME / REGISTRY_FILENAME,
...
def ensure_layout(base_path: Path) -> dict[str, Path]:
    ...
    p = paths(base_path)
    for key in ("jump_root", "ssh_dir", "registry_dir", "compose_dir"):
        try:
...
def render_compose_yaml(
    *,
    ...
    if not registry_dir.is_dir():
        raise JumpConfigError(f"jump registry directory does not exist: {registry_dir}")
    ...
    registry_scalar = json.dumps(f"{registry_dir}:{REGISTRY_MOUNT}:ro").replace("$", "$$")
