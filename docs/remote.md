...
./djinn jump refresh    # rebuild the bottle picker registry by hand
./djinn jump status
./djinn jump logs -f
./djinn jump stop
```
...
**Which UDP ports.** mosh inside the jump uses `60000:60010` (`START:END`,
inclusive). Override with `DJINN_JUMP_MOSH_PORTS` in `./.env`; the range is
container-internal (nothing is published to the host) so it only needs to
change if it collides with something else you run on the jump's address.
Takes effect on the next `./djinn jump start`.

**Trying the picker outside the jump.** `jump/picker.py` reads the registry
from `/etc/djinn-jump/registry/bottles`; set `DJINN_JUMP_REGISTRY` to point
it at any file with one `djinn-<bottle>` name per line, e.g. to drive it on
the host against a scratch list. The compose file never sets it — inside the
jump the default path is the only one the host writes.

**Bottle host keys.** A bottle's sshd host keys live on its `ssh-host-keys`
named volume (mounted at `/etc/ssh/host_keys`, generated by the entrypoint
on first boot), so a rebuild or recreate keeps the same fingerprint and the
jump's `StrictHostKeyChecking accept-new` against its persisted
`known_hosts` just works. Only `./djinn down <bottle> --purge` (which drops
the bottle's volumes) mints new keys. The first `up` of a bottle created
before this volume existed also mints them (the image-layer keys it served
until then are abandoned). In either case, clear the stale entry once:
