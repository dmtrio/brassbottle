...
eval "$DERIVED"
[ "$HOST_MCP_PORTS" = "8811" ] \
    && pass "gateway host_port folds into HOST_MCP_PORTS (broker port gone: it is the compose singleton)" \
    || fail "HOST_MCP_PORTS wrong: '$HOST_MCP_PORTS'"
[ "$EGRESS_BROKER_HOST" = "172.30.0.252" ] \
    && pass "egress broker address derived for the firewall grant" \
    || fail "EGRESS_BROKER_HOST wrong: '$EGRESS_BROKER_HOST'"
...
# ./djinn allow --watch must be gone: the egress broker now runs as a
# compose singleton, so the removed watcher arm has to name the replacement
# command instead of exec-ing a watcher that no longer exists.
grep -qF -- 'allow --watch was removed: run ./djinn egress start' djinn \
    && pass "djinn allow --watch names the compose singleton in its removal message" \
    || fail "djinn allow --watch no longer names ./djinn egress start (update this suite!)"
...
# logic unit-tested by tests/bash.test.sh).
grep -qF -- '. "$SCRIPT_DIR/src/git_identity.sh"' up.sh \
    && grep -qF -- 'apply_repo_identity "$CNAME" "$RNAME" "$RURL" "$GIT_HOST_IDENTITIES"' up.sh \
